...
class AppViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // --- Business / Company Profile (Invoices, Branding, FSSAI, Address, Phone) ---
    ...
    private val _businessProfile = MutableStateFlow(
        BusinessProfile(
            companyName = prefs.getString("business_company_name", "") ?: "",
            brandName = prefs.getString("business_brand_name", "") ?: "",
            address = prefs.getString("business_company_address", "") ?: "",
            phoneNumber = prefs.getString("business_company_phone", "") ?: "",
            fssaiNumber = prefs.getString("business_fssai_number", "") ?: ""
        )
    )
    ...
    fun saveBusinessProfile(
        ...
        prefs.edit()
            .putString("business_company_name", companyName.trim())
            .putString("business_brand_name", brandName.trim())
            .putString("business_company_address", address.trim())
            .putString("business_company_phone", phoneNumber.trim())
            .putString("business_fssai_number", fssaiNumber.trim())
            .apply()
    ...
    fun sharePaymentInvoice(context: Context, invoice: PaymentInvoice, salesEntries: List<SalesEntry>) {
        ...
        if (profile.brandName.isNotBlank()) {
            sb.appendLine("      ${profile.brandName.uppercase()}      ")
        }
        ...
        if (profile.fssaiNumber.isNotBlank()) {
            sb.appendLine("FSSAI Lic No: ${profile.fssaiNumber}")
        }
        sb.appendLine("================================")
        ...
        sb.appendLine("================================")
        if (profile.brandName.isNotBlank() || profile.companyName.isNotBlank()) {
            sb.appendLine("Issued by ${if (profile.brandName.isNotBlank()) profile.brandName else profile.companyName}")
        } else {
    ...
    fun setSalesPendingTodayFilter() {
        setPendingSalesFilter()
    }

    fun setSalesPendingOnlyFilter() {
        setPendingSalesFilter()
    }
    ...
    fun extractCoordinatesFromText(text: String): Pair<Double, Double>? {
        return com.example.utils.LocationUtils.extractCoordinates(text)
    }
    ...
    fun importAllUnifiedFromExcel(context: Context, uri: Uri) {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                // 8g. Business Profile (Company Name, Brand, Address, Phone, FSSAI)
                val businessProfileImported = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
                    com.example.utils.Exporter.importBusinessProfile(context, workbook)
                }
                if (businessProfileImported) {
                    reloadBusinessProfile()
                    sheetMessages.add("Business Profile: Company, Brand, Address, Phone & FSSAI imported")
                }
